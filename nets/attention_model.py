...
class AttentionModel(nn.Module):
    ...
    def forward(self, input, return_pi=False):
        ...
        cost, mask = self.problem.get_costs(input, pi, num_depots=self.num_depots)
    ...
    def _init_embed(self, input):
        ...
        if self.is_op:
            ...
            if self.num_depots > 1:
                embeddings = embeddings + (self.init_embed_depot(input['depot2'])[:, None, :],)
    ...
    def sample_many(self, input, batch_rep=1, iter_rep=1):
        ...
        return sample_many(
            ...
            lambda input, pi: self.problem.get_costs(input[0], pi, num_depots=self.num_depots),
        ...
        if self.is_op:
            return torch.cat(
                (
                    ...
                    (
                        (state.get_remaining_length().view(-1, batch_size) / state.max_length[..., 0])
    ...
    def _one_to_many_logits(self, query, glimpse_K, glimpse_V, logit_K, mask):
        ...
        # Batch matrix multiplication to compute logits (batch_size, num_steps, graph_size) -> logits = 'compatibility'
